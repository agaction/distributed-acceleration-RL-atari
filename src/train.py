...
from initializers import args_initialize, env_initialize, log_initialize, model_initialize
...
def worker(gpu, ngpus_per_node, args):
    ...
    # nvml device handle for retriving gpu memory usage
    nvmlInit()
    ...
    for update in iterator:
        ...
        if (args.rank == 0) and (T >= evaluation_offset):
            ...
            memory = '(gpu memory): {used:4.1f}/{total:4.1f} MB'.format(used=gpu_info.used/(1024*1024), total=gpu_info.total/(1024*1024))
            print('[training time: {}] {}'.format(format_time(total_time), ' --- '.join([reward_data, memory])))
        ...
        if args.rank == 0:
            ...
            gpu_info = nvmlDeviceGetMemoryInfo(gpu_handle)
            progress_data = callback(args, model, T, gpu_info.used/(1024*1024), iter_time, final_rewards, final_lengths,
                                     value_loss, policy_loss, dist_entropy, train_csv_writer, train_csv_file)
            iterator.set_postfix_str(progress_data)


    if args.rank == 0 and args.save_model_filename is not None:
