...
def callback(args, model, frames, gpu_memory, iter_time, rewards, lengths,
    ...
    if frames >= last_save:
        ...
        if csv_writer and csv_file:
            csv_writer.writerow([frames, fps, gpu_memory, total_time,
                                 rmean, rmedian, rmin, rmax, rstd,
                                 lmean, lmedian, lmin, lmax, lstd,
                                 entropy, value_loss, policy_loss])
            csv_file.flush()

    str_template = '{fps:8.2f}f/s, {gpu_memory:5.0f}MB,' \
    ...
    return str_template.format(fps=fps, gpu_memory=gpu_memory, rmin=rmin, rmax=rmax, rmean=rmean, rmedian=rmedian,
